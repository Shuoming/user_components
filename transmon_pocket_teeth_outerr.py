...
class TransmonPocketTeethR(BaseQubit):
    ...
    def make_pocket(self):
        ...
        rect_pk = draw.rectangle(p.pocket_width-pad_height, p.pocket_height)
        rect_pk_e = draw.rectangle(
            p.pocket_width, p.pocket_height-pad_height)

        circ_pk_r_up = draw.Point((p.pocket_width-pad_height) / 2, (p.pocket_height-pad_height) /
                                  2.).buffer(pad_height / 2,
                                             resolution=16,
                                             cap_style=CAP_STYLE.round)

        circ_pk_r_down = draw.Point((p.pocket_width-pad_height) / 2, -(p.pocket_height-pad_height) /
                                    2.).buffer(pad_height / 2,
                                               resolution=16,
                                               cap_style=CAP_STYLE.round)
        circ_pk_l_up = draw.Point(-(p.pocket_width-pad_height) / 2, (p.pocket_height-pad_height) /
                                  2.).buffer(pad_height / 2,
                                             resolution=16,
                                             cap_style=CAP_STYLE.round)
        circ_pk_l_down = draw.Point(-(p.pocket_width-pad_height) / 2, -(p.pocket_height-pad_height) /
                                    2.).buffer(pad_height / 2,
                                               resolution=16,
                                               cap_style=CAP_STYLE.round)
        rect_pk = draw.union(
            [rect_pk, rect_pk_e, circ_pk_r_up, circ_pk_r_down, circ_pk_l_up, circ_pk_l_down])
