...
class CapNInterdigitalPS(QComponent):
    ...
    def make(self):
        ...
        # CPW
        north_cpw = draw.LineString(
            [[0, 0], [0, -p.cap_distance]])
        ...
        south_cpw = draw.LineString(
            [[
                ...
                [
                0, -p.cap_distance_n-p.cap_distance -
                (p.cap_gap + 2 * p.cap_width + p.finger_length)
            ]])
        ...
        [cap_body, cap_etch] = draw.translate(
            [cap_body, cap_etch], 0, p.cap_shift)
